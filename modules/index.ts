...
export const prepareItems = (
    ...
    const items = products.reduce((array: IDeliveryItem[], product) => {
        ...
        const predicates = nomenclature.products.filter((pred) =>
            pred.code.includes(SKU)
        );

        const [desired] = predicates.filter((row) => {
            const group = nomenclature.groups.find(
                (pred) => pred.id === row.parentGroup
            );
            // @ts-ignore
        ...
        if (desired) {
            ...
            if (product.meta_data.length) {
                const modifiers = desired.groupModifiers
                ...
                deliveryItem.modifiers = product.meta_data.reduce(
                    (array: any[], { key, value }) => {
